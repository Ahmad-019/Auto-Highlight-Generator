...
# ═══════════════════════════════════════════════════════════
#  ML ENGINE & DATA ACQUISITION (تم ترقية المنطق هنا فقط)
# ═══════════════════════════════════════════════════════════
@st.cache_resource
...
def batch_classify_hybrid_engine(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # عزل التعليقات العربية عن الإنجليزية لضمان عدم خنق الموديل
    is_arabic_mask = df_neutral['Content'].astype(str).apply(lambda x: bool(arabic_pattern.search(x)))
    
    df_english = df_neutral[~is_arabic_mask]
    df_arabic = df_neutral[is_arabic_mask]

    # 1️⃣ تحليل الفيديوهات/التعليقات الأجنبية (جميعها بالكامل وبسرعة فائقة)
    ...
    # 2️⃣ تحليل التعليقات العربية (تحديد 400 تعليق فقط لعدم خنق الـ Google Translator)
    if not df_arabic.empty:
        df_arabic = df_arabic.copy()
        df_arabic['len'] = df_arabic['Content'].astype(str).str.len()
        df_arabic = df_arabic.sort_values(by='len', ascending=False)
        
        sample_size = min(400, len(df_arabic))
        df_arabic_sample = df_arabic.head(sample_size)
        
        for idx, row in df_arabic_sample.iterrows():
...
{t("Paste a YouTube URL into the sidebar to begin detecting golden moments.", "قم بلصق رابط يوتيوب في القائمة الجانبية للبدء في استخراج اللحظات الذهبية بدقة عالية.")}
</p>
</div>
""", unsafe_allow_html=True)
else:
